...
export default function PlatformGrid() {
	return (
		<section className="mt-10 text-center sm:mt-14" data-node-id="33:276">
			<h2 className="px-2 text-[16px] font-semibold text-grey-800 sm:text-[16px]">
				Everything You Need. All in One Platform.
			</h2>
			<div className="mt-6 grid grid-cols-1 gap-4 sm:mt-10 sm:grid-cols-2 sm:gap-6 md:grid-cols-3 xl:grid-cols-6 space-y-4">
				{platformFeatures.map((f) => (
					...
					>
						...
						<Image
							...
							className="my-3"
						/>
						<div className="text-[14px] text-grey-700 text-start">
							{f.lines.map((line) => (
								<p key={line}>{line}</p>
							))}
						</div>
